...
<p class="tip">Due to the way `v-model` handles event listeners on Android the event is not triggered until the autocomplete item is selected. Use `v-bind` and `v-on:input` instead of `v-model` to achieve the desired result.</p>
